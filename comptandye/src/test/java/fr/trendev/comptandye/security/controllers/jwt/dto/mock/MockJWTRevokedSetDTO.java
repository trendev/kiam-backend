...
public class MockJWTRevokedSetDTO implements JWTRevokedSetDTO {
    ...
    public CompletionStage<Set<JWTRecord>> getAll() {
        LOG.info("Getting all revoked token from Firestore");
    ...
    public void bulkRemoves(List<String> dtoRemoves) {
        LOG.log(Level.INFO,
                "Removing {0} revoked (expired) token from Firestore",
                dtoRemoves.size());
    }

    @Override
    public void create(JWTRecord record) {
        LOG.log(Level.INFO, "Adding revoked token {0} in Firestore",
                record.getToken());
    }

    @Override
    public void bulkCreation(Set<JWTRecord> records) {
        LOG.log(Level.INFO, "Adding {0} revoked token in Firestore",
                records.size());
    }

    @Override
    public void delete(String token) {
        LOG.log(Level.INFO, "Removing revoked token {0} from Firestore", token);
    }
